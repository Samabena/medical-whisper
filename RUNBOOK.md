...
Architecture cible : **« sandwich » CPU** — STT (faster-whisper) → agent + extracteur (Ollama)
→ TTS (Piper). **Aucun GPU requis.** Chaque serveur est un composant Docker distinct :
`backend`, `frontend`, `db`, `redis`, `stt`, `piper`, `ollama`, `proxy` (+ `migrate` one-shot).
...
### Tester la vraie chaîne vocale en local (CPU)
Ajouter le profil `voice` (démarre `stt`, `piper`, `ollama`) puis basculer `.env` :
```ini
SPEECH_AGENT=sandwich
STT_BACKEND=whisperlive     ; WHISPERLIVE_URL=ws://stt:9090
TTS_BACKEND=piper_http      ; PIPER_URL=http://piper:5000
AGENT_BACKEND=ollama
EXTRACTOR_BACKEND=ollama    ; OLLAMA_HOST=http://ollama:11434
```
```bash
docker compose --profile dev --profile voice up --build
docker compose exec ollama ollama pull llama3.1   # 1re fois : modèle agent/extracteur
```
...
### Pré-requis
- Hôte Linux avec Docker + Docker Compose. **Pas de GPU nécessaire** (tout tourne sur CPU).
  Prévoir de la RAM/CPU pour faster-whisper + Ollama (compter ≥ 8 cœurs, ≥ 16 Go conseillé).
...
WHISPERLIVE_URL=ws://stt:9090
WHISPER_MODEL=large-v3               # ou small si CPU limité
...
docker compose exec ollama ollama pull llama3.1     # 1re fois : modèle d'extraction/agent
```
- Le service `migrate` applique les migrations **avant** le démarrage du backend (bloquant).
- Le service `stt` télécharge le modèle Whisper au 1er démarrage (cache volume `whisper_models`).
...
| Logs vocaux | `docker compose logs -f stt piper ollama` |
...
| Modèle Ollama | `docker compose exec ollama ollama pull <modèle>` / `ollama list` |
...
| STT | `STT_BACKEND=stub` | `whisperlive` → service `stt` |
| TTS | `TTS_BACKEND=stub` | `piper_http` → service `piper` |
| Agent/Extraction | `scripted` / `null` | `ollama` → service `ollama` |
| Proxy/TLS | non | Caddy (service `proxy`) |
| Mot de passe admin | `ADMIN_PASSWORD` | `ADMIN_PASSWORD_HASH` (argon2) |

> Note : `SPEECH_AGENT=personaplex` (v2 S2S GPU) est **déprécié** — l'architecture retenue est
> le sandwich CPU. L'adapter reste dans le code mais n'est plus la cible de déploiement.
...
- Ollama et Piper sont **auto-hébergés** (aucun envoi de contenu clinique à un tiers).
...
| STT ne transcrit rien | `stt` prêt ? (modèle téléchargé — voir `logs stt`) ; `WHISPERLIVE_URL=ws://stt:9090` ? |
| Formulaire jamais rempli | `EXTRACTOR_BACKEND=ollama` et modèle `ollama pull` fait ? Sinon backend `null` |
| Latence élevée | CPU saturé → réduire `WHISPER_MODEL` (small), dédier des cœurs à Ollama, ou GPU Ollama externe |
